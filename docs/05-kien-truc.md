...
> **Tóm tắt**
> - Tầng: React UI → IPC có kiểu (tauri-specta) → commands (mỏng) → services (quy tắc R-01…R-11, transaction, ghi lịch sử) → repo (sqlx) → SQLite. Logic nghiệp vụ **chỉ nằm ở services Rust**.
...
flowchart TB
  subgraph WV["WebView2 — React 19"]
    ...
    HK --> CL["IPC có kiểu<br/>shared/api/bindings.ts"]
  end
  CL -- "invoke (JSON)" --> CMD
...
src/
├─ app/
│  ├─ main.tsx · providers.tsx       # QueryClient, ThemeProvider, RouterProvider
│  └─ routes/                        # TanStack Router, file-based, mỏng: chỉ ghép feature
│     ├─ __root.tsx                  # layout: Sidebar, thanh trên (tìm kiếm, Sáng/Tối, "Thêm việc"),
│     │                              #   TaskDetailPanel khi URL có ?task=id, TaskFormDialog
│     ├─ index.tsx                   # SC-1 Tổng quan
│     ├─ tasks.tsx                   # SC-2 Công việc (?q=, bộ lọc trên URL)
│     ├─ projects.$projectId.tsx     # SC-3 Dự án (?view=kanban|list)
│     ├─ employees.index.tsx         # SC-6 Nhân viên
│     ├─ employees.$employeeId.tsx   # SC-7 Chi tiết nhân viên
│     ├─ trash.tsx                   # SC-8 Thùng rác
│     └─ settings.tsx                # SC-9 Cài đặt
├─ features/
...
│  ├─ api/            # bindings.ts (SINH TỰ ĐỘNG, không sửa tay), client.ts (Result → throw AppError),
│  │                  #   query-keys.ts, errors.ts (mã lỗi → câu tiếng Việt)
...
├─ .sqlx/                          # dữ liệu offline cho macro sqlx (commit)
├─ tests/                          # test tích hợp services trên SQLite in-memory
└─ src/
   ├─ main.rs · lib.rs             # Builder: single-instance ĐẦU TIÊN, window-state, dialog, opener; setup; specta
...
DTO dùng `#[serde(rename_all = "camelCase")]`. `bindings.ts` sinh khi chạy bản debug và được commit.

## 4. Command IPC

Mọi command trả `Result<T, AppError>`. Kiểu chi tiết do Rust định nghĩa, xuất sang `bindings.ts`.
...
| | `deactivate_employee` | `{ id, reassignTo: id \| null }` → — (R-04: giao lại việc chưa xong, ghi lịch sử từng việc) |
...
**Thư mục dữ liệu** `%APPDATA%\vn.personal.quanlytask\` (identifier đã chốt, [01 §4](01-cong-nghe.md#4-đóng-gói-và-phân-phối)):
...
```rust
pub struct AppError { pub code: ErrorCode, pub message: String, pub details: Option<serde_json::Value> }
...
- Service kiểm tra quy tắc trước khi ghi, trả `Validation` kèm `details.field` để UI hiện lỗi dưới đúng trường. Không `unwrap()` ngoài test.
- `shared/api/client.ts` chuyển `Result` lỗi thành `throw AppError`. `shared/api/errors.ts` đổi mã thành câu tiếng Việt, hiện bằng toast. Lỗi render có `ErrorBoundary` theo route với nút "Tải lại".
...
- **Quyền:** một capability `main.json` cho cửa sổ duy nhất, chỉ gồm các command của app (khai báo trong `build.rs`). Không bật plugin `shell`, `fs`, `http`. `dialog` và `opener` chỉ gọi từ Rust, JS không có quyền.
...
| ADR-02 | SQLite + sqlx, không ORM, migration nhúng | Một file, kiểm tra SQL lúc biên dịch | Server DB, Diesel/SeaORM |
...
| ADR-04 | IPC có kiểu bằng tauri-specta (pin `=rc.25`), commit `bindings.ts` | Đổi Rust thì TS báo lỗi ngay | Viết tay kiểu TS |
